resource "aws_eks_cluster" "main" {
  ...
  dynamic "encryption_config" {
    for_each = var.eks[count.index].use_encryption_config == "" ? [0] : [1]
    content {
  ...
  dynamic "kubernetes_network_config" {
    for_each            = var.eks[count.index].use_kubernetes_network_config == "" ? [0] : [1]
    content {
  ...
  dynamic "outpost_config" {
    for_each                      = var.eks[count.index].use_outpost_config == "" ? [0] : [1]
    content {
