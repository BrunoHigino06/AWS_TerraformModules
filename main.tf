resource "aws_eks_cluster" "main" {
  ...
  dynamic "encryption_config" {
    for_each = var.eks[count.index].use_encryption_config == null ? [false] : [var.eks[count.index].use_encryption_config]
    content {
  ...
  dynamic "kubernetes_network_config" {
    for_each            = var.eks[count.index].use_kubernetes_network_config == null ? [false] : [var.eks[count.index].use_kubernetes_network_config]
    content {
  ...
  dynamic "outpost_config" {
    for_each                      = var.eks[count.index].use_outpost_config == null ? [false] : [var.eks[count.index].use_outpost_config == null]
    content {
